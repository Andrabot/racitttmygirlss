...
// Dirubah 
const pengirim = "sayang ෆ";
const nomorWa = "6285793682364" // awalan nomor 0 harus di awalin 62
const textWa = "Ah bisa aja kamu"
const pesan = `terimaa kasihhh sayanggg`;
...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: 'mauu balesss apaaa yaaa><',
    html: `<input type="text" id="login" class="swal2-input" placeholder="Pesan Kamu">`,
    confirmButtonText: 'Kirim',
    focusConfirm: false,
    preConfirm: () => {
      const login = Swal.getPopup().querySelector('#login').value
      if (!login) {
        Swal.showValidationMessage(`Masukin Pesan Kamu Dong,  :(`)
      }
      return { login }
    }
  }).then((result) => {
    ...
    let txt = `Halo, terimakasih udah bales pesanku dengan, ${result.value.login},  ${namaAwal.toUpperCase()}, ${pesan}`;
